...
namespace Pondo
{
    ...
    bool SceneSerializer::Load(Scene* scene, const std::string& path, const std::shared_ptr<Shader>& shader)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            if (token == "BeginEntity") { name = "Entity"; meshType = "Cube"; pos = {}; rot = {}; scale = { 1,1,1 }; color = { 1,1,1,1 }; }
            else if (token == "Name") { std::getline(ss, name); if (!name.empty() && name[0] == ' ') name = name.substr(1); }
            ...
            else if (token == "EndEntity")
            {
                ...
                e->SetMaterial(mat);
                e->GetMaterial()->Mat->SetColor(color);  // ← match SpawnEntity pattern
            }
        }
